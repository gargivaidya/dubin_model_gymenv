...
class DubinGym(gym.Env):
	...
	def __init__(self, start_point):
		...
		self.cordinates = [start_point[0], start_point[1], 0]
		head_to_target = self.get_heading([self.cordinates[0]/MAX_X, self.cordinates[1]/MAX_Y], self.target)
		self.cordinates[2] = start_point[2] - head_to_target
		self.traj_x = [self.cordinates[0]]
		self.traj_y = [self.cordinates[1]]
		self.traj_yaw = [self.cordinates[2]]
		self.pose = [abs(self.cordinates[0]/MAX_X - self.target[0]), abs(self.cordinates[1]/MAX_Y - self.target[1]), self.cordinates[2]]
	...
	def reset(self): 
		...
		self.pose = np.array([abs(x/MAX_X - self.target[0]), abs(y/MAX_Y - self.target[1]), yaw])
		self.traj_x = [x]
		self.traj_y = [y]
		self.traj_yaw = [yaw]
		self.cordinates = [x, y, yaw]
	...
	def get_reward(self):
		...
		x = self.cordinates[0]/MAX_X
		y = self.cordinates[1]/MAX_Y
		yaw_car = self.cordinates[2]
		head_to_target = self.get_heading([x, y, yaw_car], self.target)
		"""
		...
		ld = self.get_distance([x, y, yaw_car], self.target)
		crossTrackError = math.sin(alpha) * ld

		return -1*(abs(crossTrackError)**2 + self.pose[0] + self.pose[1] + 3*abs(yaw_car - head_to_target)/1.57)/6
	...
	def step(self,action):
		...
		yaw_car = self.cordinates[2]
		self.action = action
		next_cordinates = self.update_state([x, y, yaw_car], action, 0.005) # 0.005 Modify time discretization
		self.cordinates = [next_cordinates[0]*MAX_X, next_cordinates[1]*MAX_Y, next_cordinates[2]]
		self.pose = [abs(next_cordinates[0] - self.target[0]), abs(next_cordinates[1] - self.target[1]), next_cordinates[2]]

		if ((abs(self.pose[0]) < 1.) and (abs(self.pose[1]) < 1.)):

			if(self.pose[0]<THRESHOLD_DISTANCE_2_GOAL and self.pose[1]<THRESHOLD_DISTANCE_2_GOAL):
				reward = 10            
				done = True
				#print('Goal Reached')
	...
	def render(self):
		self.traj_x.append(self.cordinates[0])
		self.traj_y.append(self.cordinates[1])
		self.traj_yaw.append(self.cordinates[2])
	...
	def plot_car(self, cabcolor="-r", truckcolor="-k"):  # pragma: no cover
		# print("Plotting Car")
		x = self.cordinates[0] #self.pose[0]
		y = self.cordinates[1] #self.pose[1]
		yaw = self.cordinates[2] #self.pose[2]
